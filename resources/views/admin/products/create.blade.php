    ...
    <div class="relative min-h-screen overflow-hidden">
        ...
        <main class="relative z-10">
            <section class="mx-auto max-w-4xl px-6 py-10">
                ...
                <h1 class="mt-2 text-3xl font-extrabold tracking-tight">New Product</h1>
                ...
                <form class="mt-8 space-y-6 rounded-3xl bg-white/70 p-8 ring-1 ring-slate-200 backdrop-blur"
                    ...
                    <div>
                        ...
                        <div class="relative mt-2">
                            ...
                            <input id="priceInput" name="price" type="text" inputmode="numeric" autocomplete="off"
                                required value="{{ old('price') }}" placeholder="0"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            class="mt-2 w-full rounded-2xl bg-white px-4 py-3 text-sm text-slate-900 ring-1 ring-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-600">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input name="image" type="file" accept="image/*"
                            class="mt-2 w-full rounded-2xl bg-white px-4 py-3 text-sm text-slate-700 ring-1 ring-slate-200 file:mr-4 file:rounded-full file:border-0 file:bg-slate-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-slate-700 hover:file:bg-slate-200" />
                        @error('image')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-2 text-xs text-slate-500">Foto akan disimpan ke folder <span
                                class="font-semibold">public/images/products</span>.</p>
                    </div>

                    <label class="flex items-center gap-3 text-sm text-slate-700">
                        <input name="is_active" type="checkbox" value="1" {{ old('is_active') ? 'checked' : '' }}
